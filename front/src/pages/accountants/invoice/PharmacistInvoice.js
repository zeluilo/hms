...
const PharmacistInvoice = () => {
    ...
    useEffect(() => {
        ...
        const fetchPaymentDetails = async () => {
            try {
                const response = await axios.get(`http://localhost:3001/thomas/get-Payment-Details/${paymentId}`);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Filter prescriptions for the specified paymentId
                const prescriptionsForPaymentId = groupedPrescriptions[paymentId] || [];

                // Convert prescriptions for the paymentId into an array with paymentId and prescriptions details
                const prescriptionsArray = [{
                    paymentId,
                    prescriptions: prescriptionsForPaymentId,
                }];
                    ...
                    <div class="invoice-box">
                        ...
                        <div class="invoice-item">
                            <table>
                                ...
                                allPrescriptions.forEach((prescriptionGroup, index) => {
                                    prescriptionGroup.prescriptions.forEach((prescription, prescriptionIndex) => {
                                        console.log('All prescriptions:',prescription);
                                        printWindow.document.write(`
                                            <tr key=${prescriptionIndex} class="item">
                                                ...
                                                <div className="col-7">
                                                    <div className="card info-card sales-card invoice-details" style={{ padding: '30px', border: '0.5px solid rgba(204, 204, 204, 0.5)', borderRadius: '15px' }}>
                                                        <table className='table table-hover' style={{ padding: '20px' }} cellPadding="0" cellSpacing="0">
                                                            ...
                                                            <tbody>
                                                                {allPrescriptions.map((prescriptionGroup, index) => (
                                                                    prescriptionGroup.prescriptions.map((prescription, prescriptionIndex) => (
