...
const DoctorInvoice = () => {
    ...
    const [allinvestigations, setinvestigations] = useState([]);
    const [loading, setLoading] = useState(true);
    const { currentUser } = useAuth();
    console.log("currentUser:", currentUser);

    useEffect(() => {
        console.log('Booking ID:', paymentId); // Log the bookingId
        const fetchPaymentDetails = async () => {
            try {
                const response = await axios.get(`http://localhost:3001/thomas/get-investigation-payment-details/${paymentId}`);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Filter investigations for the specified paymentId
                const investigationsForPaymentId = groupedPaidinvestigations[paymentId] || [];

                // Convert investigations for the paymentId into an array with paymentId and investigations details
                const investigationsArray = [{
                    paymentId,
                    investigations: investigationsForPaymentId,
                }];
                    ...
                    <div class="invoice-box">
                        ...
                        <div class="invoice-item">
                            <table>
                                ...
                                allinvestigations.forEach((investigationGroup, index) => {
                                    investigationGroup.investigations.forEach((investigation, investigationIndex) => {
                                        console.log('All investigations:',investigation);
                                        printWindow.document.write(`
                                            <tr key=${investigationIndex} class="item">
    ...
    return (
        <main id="main" className="main">
            <div className="pagetitle">
                <h1 style={{fontSize:'2rem'}}>Invoice</h1>
            </div>
                ...
                <div className="row">
                    <div className="col-lg-12">
                        ...
                        <div className="row justify-content-end">
                            ...
                            {/* Print button */}
                            <div className="col-4" style={{ display: 'flex', alignItems: 'center', width: '150px' }}>
                                <div onClick={print_invoice} className="card info-card sales-card" style={{ padding: '10px', textAlign: 'center', cursor: 'pointer', backgroundColor: '#f0f8fa', border: '2.5px dotted #000', width: '100%' }}>
                                    <span style={{ textAlign: 'left', paddingLeft: '15px' }}>Print<i className="bi bi-printer" style={{ paddingLeft: '15px' }}></i></span>

                                </div>
                            </div>

                            {/* Download button */}
                            <div className="col-4" style={{ display: 'flex', alignItems: 'center', width: '200px' }}>
                                <div onClick={download_invoice} className="card info-card sales-card" style={{ padding: '10px', textAlign: 'center', cursor: 'pointer', backgroundColor: '#f0f8fa', border: '2.5px dotted #000', width: '100%' }}>
                                    <span style={{ textAlign: 'left', paddingLeft: '15px' }}>Download<i className="bi bi-download" style={{ paddingLeft: '15px' }}></i></span>
                                </div>
                            </div>
                                                        ...
                                                        <table className='table table-hover' style={{ padding: '20px' }} cellPadding="0" cellSpacing="0">
                                                            <thead>
                                                                <tr className="heading" style={{ fontSize: '1.2rem' }}>
                                                                    <td><strong>Laboratory Test</strong></td>
                                                                    <td><strong>Price</strong>(₦)</td>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                {allinvestigations.map((prescriptionGroup, index) => (
                                                                    prescriptionGroup.investigations.map((prescription, prescriptionIndex) => (
                                                                        <tr key={prescriptionIndex} className="details">
                    ...
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                <div><button onClick={sendEmail} style={{ width: '100%', textAlign: 'left' }} className="btn btn-outline-success"><i style={{ padding: '15px' }} className="bi bi-envelope"></i>Send to Email</button></div>
                                <br />
                                <div><button onClick={sendSMS} style={{ width: '100%', textAlign: 'left' }} className="btn btn-outline-success"><i style={{ padding: '15px' }} className="bi bi-telephone-fill"></i>Send to Phone Number</button></div>
                            </div>
            ...
            <div className="modal fade" id="generate" tabIndex="-1">
